...
#endif  /* USE_DHCP */
#ifdef USE_LOG
   UsrLog ("The network cable is not connected \r\n");
#else    
...
#ifdef USE_DHCP
#ifdef USE_LOG
    UsrLog ("The network cable is now connected \r\n");
#else
    BSP_LED_Off(LED2);
    BSP_LED_On(LED1);
#endif /* USE_LOG */
  ...
  case DHCP_START:
    {
      ...
      dhcp_start(netif);
#ifdef USE_LOG
      UsrLog ("  State: Looking for DHCP sever ...\r\n");
#endif
    }
    break;
  ...
  case DHCP_WAIT_ADDRESS:
    {
      ...
      if (IPaddress !=0) 
      {
        ...
        UsrLog ("IP address assigned by a DHCP server: %s\r\n", iptxt);
#else
     BSP_LED_On(LED1);   
#endif
      }
